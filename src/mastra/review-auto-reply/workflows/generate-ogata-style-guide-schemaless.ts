...
const extractionSchema = z.string().nullable().describe('スタイルガイド');
...
const formatStyleGuide = new Step({
  ...
  execute: async ({ context }) => {
    const styleGuide = context.getStepResult(generateStyleGuide);

    const csvFileName = context.triggerData?.csvFileName;
    const timestamp = Date.now();
    const fileName = `style-guide-${csvFileName}-${timestamp}.md`;
    const filePath = path.resolve(__dirname, '../../data/output/schemaless', fileName);

    await fs.writeFile(filePath, styleGuide ?? '', 'utf-8');
    console.log(`Style guide written to ${filePath}`);
    return { filePath };
  },
});

const generateOgataStyleGuideSchemalessWorkflow = new Workflow({
  name: 'generate-ogata-style-guide-schemaless-workflow',
...
generateOgataStyleGuideSchemalessWorkflow.commit();

export { generateOgataStyleGuideSchemalessWorkflow };
